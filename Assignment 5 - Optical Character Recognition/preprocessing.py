...
def sobel_filters(X_train):
    ...
    for x in X_train:
        grad_x = ndimage.filters.convolve(x, K_x)
        grad_y = ndimage.filters.convolve(x, K_y)
...
def hog_descriptor(X_train):
    ...
    for x in X_train:
        ...
        f, _ = hog(x, orientations=8, pixels_per_cell=(8,8), cells_per_block=(1,1), visualize=True, multichannel=False, feature_vector=True, block_norm='L2-Hys') # orientations = number of feature buckets
        feature_vector.append(f)
    return feature_vector



def sobel_transform(X_train, X_test):
    X_train, _ = sobel_filters(X_train)
    X_test, _ = sobel_filters(X_test)
